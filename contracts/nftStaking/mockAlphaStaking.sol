...
contract alphaStaking is Ownable {
    ...
    constructor(
        ...
        require(_stakeToken != address(0), "alphaStaking: stake token address is 0");
    ...
    function addPool(uint _stakingCap, uint _fixedStakingAmount, uint _rewardAPY, uint _startTime, uint _endTime) public onlyOwner {
        require(_startTime < _endTime, "alphaStaking: endTime > startTime");
    ...
    function stake(uint _pid, uint _amount) public returns (bool) {
        ...
        require(_amount == pools[_pid].fixedStakingAmount, "alphaStaking: wrong amount");
        require(stakes[_pid][staker].amount == 0, "alphaStaking: already staked");
        require(getTimeStamp() >= pools[_pid].startTime, "alphaStaking: bad timing request");
        require(getTimeStamp() < pools[_pid].endTime, "alphaStaking: bad timing request");

        require(pools[_pid].stakedTotal.add(_amount) <= pools[_pid].stakingCap, "alphaStaking: Staking cap is filled");
    ...
    function withdraw(uint _pid) public returns (bool) {
        ...
        require(getTimeStamp() > pools[_pid].endTime, "alphaStaking: bad timing request");
        require(amount > 0, "alphaStaking: nothing to withdraw");
        require(claim(_pid), "alphaStaking: claim error");
    ...
    function withdrawInternal(uint _pid, address _staker, uint _amount) internal returns (bool) {
        require(_amount > 0, "alphaStaking: must be positive");
        require(_amount <= stakes[_pid][msg.sender].amount, "alphaStaking: not enough balance");
